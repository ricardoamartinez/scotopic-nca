...
def create_sparse_mask(
    ...
    Args:
        frame: A single frame tensor of shape (B, C, H, W).
    ...
    num_to_keep = int(num_pixels_per_channel * keep_fraction)
    
    mask = torch.zeros_like(frame)
    for i in range(b):
        for j in range(c):
            indices = torch.randperm(num_pixels_per_channel, device=frame.device)[:num_to_keep]
            flat_mask = torch.zeros(num_pixels_per_channel, device=frame.device)
            flat_mask[indices] = 1.0
            mask[i, j] = flat_mask.view(h, w)
